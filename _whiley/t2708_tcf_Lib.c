...
*/
static void cf_lib_xTaskCreate(func stabilizerTask, char* stabilizerName, int depth, long n1, int priority, long n2){
...
char * cf_lib_getRollType() { char *a = rollType == 0 ? "ANGLE" : rollType == 2 ? "RATE" : "error"; return a;}
char * cf_lib_getPitchType(){ char *a = pitchType == 0 ? "ANGLE" : pitchType == 2 ? "RATE" : "error"; return a;}
char * cf_lib_getYawType()  { char *a = yawType == 0 ? "ANGLE" : yawType == 2 ? "RATE" : "error"; return a;}
